...
class Skill(models.Model):
    ...
    requires_verification = models.BooleanField(
        default=False, verbose_name="Nécessite une vérification"
    )
...
class VolunteerSkill(models.Model):
    ...
    status = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="VALIDATED", verbose_name="Statut"
    )

    # Pour les compétences nécessitant une vérification
    document = models.FileField(
        upload_to="skill_documents/", null=True, blank=True, verbose_name="Document justificatif"
    )
