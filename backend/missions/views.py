...
from .models import Mission, Application, Participation, Review
    ...
    def get_queryset(self):
        ...
        if has_places == 'true':
            queryset = queryset.filter(
                accepted_volunteers__lt=models.F('required_volunteers')
            )
...
class VolunteerDashboardView(APIView):
    ...
    def get(self, request):
        ...
        accepted_missions = Application.objects.filter(
            volunteer=volunteer,
            status='ACCEPTED',
            mission__date__gte=timezone.now().date()
        ).count()
        ...
        recent_applications = Application.objects.filter(
            volunteer=volunteer
        ).select_related('mission__organization')[:5]
...
class ApplyToMissionView(APIView):
    ...
    def post(self, request, mission_id):
        ...
        volunteer_skills = volunteer.skills.filter(status='VALIDATED').values_list('skill_id', flat=True)
        
        has_all_skills = all(
            req.skill_id in volunteer_skills 
...
class MyApplicationsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        ).select_related('mission__organization').order_by('-applied_at')
...
class OrganizationDashboardView(APIView):
    ...
    def get(self, request):
        ...
        pending_applications = Application.objects.filter(
            mission__organization=organization,
            status='PENDING'
        ).count()
...
class OrganizationMissionsView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        ).select_related('odd').order_by('-created_at')
...
class MissionApplicationsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Application.objects.filter(
            ...
            mission__organization=self.request.user.organization_profile
        ).select_related('volunteer__user').order_by('-applied_at')
    ...
    def post(self, request, application_id):
        try:
            application = Application.objects.select_related('mission').get(
                id=application_id,
                mission__organization=request.user.organization_profile
            )
